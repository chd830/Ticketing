...
public class HomeController {
	...
	public ModelAndView use_discount(@RequestParam int discountPrice,@RequestParam int inwon, HttpServletRequest req) {
		...
		System.out.println(stdto.getDiscountPrice());
		...
		mv.addObject("performReaddto",performReaddto);
		mv.addObject("Read_placedto",Read_placedto);
	...
	public String why(HttpServletRequest req, HttpServletResponse response) {
		...
		stdto = dao.selectTicket(userId);
		List<SelectedTicketDTO> list = dao.seatSelectTicket(userId);
		...
		int reservationNum = rd.nextInt(9999999) + 1;
		
		while(reservationNum<1000000)
			reservationNum = rd.nextInt(9999999) + 1;
		
		int al = rd.nextInt(26);
		
		String reserNum = (String)alpha[al]+reservationNum;
		...
		req.setAttribute("reserNum", reserNum);
		req.setAttribute("adto", adto);
		req.setAttribute("stdto", stdto);
		req.setAttribute("cancelDate", cancelDate);
		req.setAttribute("deadline", deadline);
		req.setAttribute("list", list);
		req.setAttribute("performReaddto", performReaddto);
		req.setAttribute("Read_placedto", Read_placedto);
